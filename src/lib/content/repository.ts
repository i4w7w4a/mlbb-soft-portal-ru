...
export function createContentRepository(options?: { contentRoot?: string }) {
  ...
  const loadTaxonomy = cache(async () => {
    const [tagsRaw, categoriesRaw] = await Promise.all([
      readJson<{ tags: TaxonomyTag[] }>(path.join(contentRoot, "taxonomy", "tags.json")),
      readJson<{ categories: TaxonomyCategory[] }>(
        path.join(contentRoot, "taxonomy", "categories.json"),
      ),
    ]);
  ...
  const loadHeroes = cache(async () => {
    const index = await readJson<{ heroes: string[] }>(path.join(heroRoot, "index.json"));
    const heroes = await Promise.all(
      index.heroes.map(async (slug) =>
        heroSchema.parse(await readJson(path.join(heroRoot, slug, "hero.json"))),
      ),
    );
    ...
    const allNews = (
      await Promise.all(
        heroes.map(async (hero) => {
          const newsDirectory = path.join(heroRoot, hero.slug, "news");
  ...
  const loadLatestIndex = cache(async () => {
    return latestIndexSchema.parse(
      await readJson<LatestIndex>(path.join(contentRoot, "news", "latest-index.json")),
    );
  });

  const loadSiteSettings = cache(async () => {
    return siteSettingsSchema.parse(
      await readJson<SiteSettings>(path.join(contentRoot, "site", "settings.json")),
    );
  });

  const loadSoftConfig = cache(async () => {
    return softConfigSchema.parse(
      await readJson<SoftConfig>(path.join(contentRoot, "site", "soft.json")),
    );
  });
  ...
  async function getFeaturedHeroes() {
    const heroes = await loadHeroes();
  ...
  async function getHeroNews(heroSlug: string, includeDrafts = false) {
  ...
  async function getAllNews(includeDrafts = false) {
    const news = await loadNews();
  ...
  async function getSpotlightNews() {
    const [news, latestIndex] = await Promise.all([getAllNews(), loadLatestIndex()]);
    const newsMap = new Map(news.map((story) => [story.id, story]));
  ...
  async function getNewsBySlug(slug: string, includeDrafts = false) {
    const news = await getAllNews(includeDrafts);
    return news.find((story) => story.slug === slug) ?? null;
  }

  async function getRelatedHeroes(hero: Hero) {
  ...
  async function getRelatedNews(story: News) {
    const news = await getAllNews();
  ...
  async function getPreviousNextNews(slug: string) {
    const news = await getAllNews();
